...
export class AHPClient implements AHPClientBlueprint {
    public checkIsSlash = (str: string): Boolean | undefined => {
      if (str.length > 2) {
        if (str.includes("/")) {
    ...
    public convertMatrix = (matrix: MatrixType): ConvertedMatrixResult => {
    ...
    public calcPriority = (matrix: ConvertedMatrix): number[] => {
      const u = [];
      const w = [];
    ...
    public calcIntensity = (matrix: ConvertedMatrix, w: number[]): number[] => {
      const intensity = [];
    ...
    public calcFraction = (w: number[], intensity: number[]): number[] => {
      const fraction = [];
    ...
    public calcLambda = (fraction: number[]): number => {
      const lambda =
    ...
    public calcInd = (lambda: number, length: number): number => {
      return Number(((lambda - length) / (length - 1)).toFixed(6));
    };
  
    public calcRelativeCoherence = (ind: number, consistInd: number): number => {
      return Number((ind / consistInd).toFixed(6));
    };
  
    public calcMatrixResults = (
      matrix: MatrixType
