...
describe('AHPClient', () => {
  describe('checkIsSlash', () => {
    it('returns true for valid slash format', () => {
      expect(ahp.checkIsSlash('3/4')).toBe(true);
    });

    it('returns false for leading slash', () => {
      expect(ahp.checkIsSlash('/4')).toBe(false);
    });

    it('returns false for slash too short', () => {
      expect(ahp.checkIsSlash('3/')).toBe(false);
    });

    it('returns true for non-slash short input', () => {
      expect(ahp.checkIsSlash('2')).toBe(true);
    });
  });

  describe('convertMatrix', () => {
    it('converts valid matrix with slashes', () => {
      const matrix: MatrixType = [['1', '1/2'], ['2', '1']];
      const { convertedMatrix, isError } = ahp.convertMatrix(matrix);
    ...
    it('returns error for invalid matrix entry', () => {
      const matrix: MatrixType = [['1', '/2'], ['2', '1']];
      const { isError } = ahp.convertMatrix(matrix);
  ...
  describe('calcPriority', () => {
    it('calculates weight vector', () => {
      ...
      const result = ahp.calcPriority(converted);
  ...
  describe('calcResults', () => {
    it('aggregates multiple matrices', () => {
      ...
      const result = ahp.calcResults(matrixes);
      if (typeof result !== 'string') {
