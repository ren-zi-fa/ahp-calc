...
AHPClient.convertMatrix = (matrix) => {
    const convertedMatrix = [];
    let isError = false;
    for (let i = 0; i < matrix.length; i++) {
        const mi = [];
        for (let j = 0; j < matrix[i].length; j++) {
            if (_a.checkIsSlash(matrix[i][j])) {
...
AHPClient.calcIntensity = (matrix, w) => {
    const intensity = [];
    for (let i = 0; i < matrix.length; i++) {
...
AHPClient.calcFraction = (w, intensity) => {
    const fraction = [];
...
AHPClient.calcLambda = (fraction) => {
    const lambda = fraction.reduce((sum, record) => sum + record, 0) /
        fraction.length;
    return Number(lambda.toFixed(6));
};
AHPClient.calcInd = (lambda, length) => {
    return Number(((lambda - length) / (length - 1)).toFixed(6));
};
AHPClient.calcRelativeCoherence = (ind, consistInd) => {
    return Number((ind / consistInd).toFixed(6));
};
AHPClient.calcMatrixResults = (matrix) => {
    const { convertedMatrix, isError } = _a.convertMatrix(matrix);
    if (!isError) {
        const consistInd = constant_1.avgConsistInd[convertedMatrix.length - 3];
        const w = _a.calcPriority(convertedMatrix);
        const intensity = _a.calcIntensity(convertedMatrix, w);
        const fraction = _a.calcFraction(w, intensity);
        const lambda = _a.calcLambda(fraction);
        const ind = _a.calcInd(lambda, convertedMatrix.length);
        const coherence = _a.calcRelativeCoherence(ind, consistInd);
        return { w, intensity, fraction, lambda, ind, coherence, consistInd };
    }
    else
        return "Something went wrong";
};
AHPClient.calcResults = (matrixes) => {
    let isError = false;
    const result = matrixes.map((matrix) => {
        const res = _a.calcMatrixResults(matrix);
